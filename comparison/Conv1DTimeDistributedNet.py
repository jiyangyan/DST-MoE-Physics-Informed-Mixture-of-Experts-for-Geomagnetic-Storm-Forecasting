...
class Conv1DTimeDistributedNet(nn.Module):
    def __init__(self, input_dim, timesteps):
        super(Conv1DTimeDistributedNet, self).__init__()
        self.timesteps = timesteps
...
# 创建测试模型
test_model = Conv1DTimeDistributedNet(input_dim, timesteps)
...
# 模型初始化
model = Conv1DTimeDistributedNet(input_dim, timesteps).to(device)
print("模型初始化完成！\n")
...
scheduler = ReduceLRBacktrack(
        model=model,
        best_model_path='comparison_checkpoints/cnn.pth',
        optimizer=optimizer,
        monitor='val_loss',
        factor=0.5,
        patience=5,
...
# 加载最佳模型
def load_best_model(model_path='comparison_checkpoints/cnn.pth'):
    checkpoint = torch.load(model_path, map_location=device)
    model = Conv1DTimeDistributedNet(input_dim=len(XCOLS), timesteps=data_config["timesteps"]).to(device)
