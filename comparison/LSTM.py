...
class LSTMModel(nn.Module):
    def __init__(self, input_dim, timesteps, n_neurons=512, dropout=0.4):
...
# 创建测试模型
test_model = LSTMModel(input_dim, timesteps)
...
# 模型初始化
model = LSTMModel(input_dim, timesteps).to(device)
print("模型初始化完成！\n")
...
scheduler = ReduceLRBacktrack(
        model=model,
        best_model_path='comparison_checkpoints/lstm.pth',
        optimizer=optimizer,
        monitor='val_loss',
        factor=0.5,
        patience=5,
...
# 加载最佳模型
def load_best_model(model_path='comparison_checkpoints/lstm.pth'):
    checkpoint = torch.load(model_path, map_location=device)
    model = LSTMModel(input_dim=len(XCOLS), timesteps=data_config["timesteps"]).to(device)
