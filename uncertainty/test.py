...
def test_moe_system(moe_system, test_sequences, test_labels, mode='adaptive', collect_quantiles=False):
    ...
    abnormal_mask = (all_labels < -50).any(dim=1)
    normal_mask = ~abnormal_mask

    if normal_mask.any():
        normal_rmse = torch.sqrt(criterion(all_predictions[normal_mask], all_labels[normal_mask])).item()
    else:
        normal_rmse = float('nan')

    if abnormal_mask.any():
        abnormal_rmse = torch.sqrt(criterion(all_predictions[abnormal_mask], all_labels[abnormal_mask])).item()
    else:
        abnormal_rmse = float('nan')
...
def save_final_results(all_results, baseline_rmse=10.601628):
    """保存最终结果"""
    print("\n" + "="*70)
    print("保存最终结果")
    print("="*70)
...
def main():
    ...
    # 保存最终结果
    save_final_results(all_results, baseline_rmse=10.601628)
